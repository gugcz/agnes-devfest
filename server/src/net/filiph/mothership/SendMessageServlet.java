...
public class SendMessageServlet extends BaseServlet {
	...
	static final String PARAMETER_MESSAGE_TEXT = "messageText";
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		String messageText = req.getParameter(PARAMETER_MESSAGE_TEXT);
		if (multicastKey != null) {
			sendMulticastMessage(multicastKey, resp, messageText);
			return;
		}
	...
	private void sendMulticastMessage(String multicastKey, HttpServletResponse resp, String messageText) {
		// Recover registration ids from datastore
		List<String> regIds = Datastore.getMulticast(multicastKey);
		Message message = null;
		try {
			// little hacky but I couldn't send utf-8 characters any other way
			byte[] bytes = Base64.encode(messageText.getBytes("utf-8"));
			message = new Message.Builder().addData("message", new String(bytes, "utf-8")).build();
